...
cps_svy <- data %>%
  ...
  select(-asecflag, -serial, -month, -cpsid, -cpsidp, -pernum, -asecwth) %>%
  ...
  mutate_at(vars(county, pubhous, ownershp, empstat, labforce, diffany, workly, poverty, himcarely, phinsur, kidcneed, union, yrimmig, foodstmp, stampval), list(~ case_when( . == 0 ~ NA_real_, TRUE ~ as.numeric(.)))) %>%
  #should occ/occly and ind/indly be in here? I feel like no, but just want to double check
...
#HIMCAIDLY - covered by Medicaid last year only exists for 2019, not 2018
#YRIMMIG - what to do? throw away 2005, 2009, 2013?
  ...
  mutate_at(vars(pubhous, foodstmp, sex, vetstat, labforce, diffany, himcaidly, himcarely, phinsur, anycovnw, kidcneed, workly), list(~ case_when(
    . == 2 ~ 1,
    . == 1 ~ 0,
    TRUE ~ NA_real_))) %>%
...
#NOTE TO GROUP: we can perform any operation on this object as we normally would on a typical df. See example in next code chunk, where I find the mean age in the object
weighted_cps <- as_survey_design(cps_svy, weights = asecwt)

#Create tibbles by year to use in model construction
#NOTE TO GROUP: I am doing this assuming that we will train the model on 2018 data and predict on 2019 data
